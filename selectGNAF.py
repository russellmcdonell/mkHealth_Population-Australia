...
SYNOPSIS
$ python selectGNAF.py [-I GNAFinputfile|--GNAFinputfile=GNAFinputfile] -O GNAFoutputfile|--GNAFoutputfile=GNAFoutputfile
                       -n addresses|--addresses=addresses [-s states|--states=states]
					   [-v loggingLevel|--loggingLevel=loggingLevel] [-o logfile|--logfile=logfile]
...
The main code
	...
	with zipfile.ZipFile('MB_2021_AUST.zip') as zf:
	...
	OTstates = {'2':'NSW', '6':'WA'}
	inCount = outCount = 0
	with open(GNAFinputfile, 'rt', encoding='utf-8-sig') as gnafCore:
		...
		for row in gnafReader:
			...
			if header:
				...
				if noOfAddresses is None:
					gnafSelection = open(GNAFoutputfile, 'wt', encoding='utf-8-sig')
	...
	SA1list = list(SA1s)
	with open(GNAFoutputfile, 'wt', encoding='utf-8-sig') as gnafSelection:
		gnafWriter = csv.DictWriter(gnafSelection, fieldnames=heading, delimiter='|')
		gnafWriter.writeheader()
